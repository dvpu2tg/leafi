...
static VALUE_TYPE const NORMAL_BREAKPOINTS_8[255] = {
...
void extractBreakpoints8(VALUE_TYPE *breakpoints, VALUE_TYPE const *values, unsigned int length) {
    VALUE_TYPE *current_breakpoint;
...
void *getAdhocBreakpoints8Thread(void *cache) {
    ...
    VALUE_TYPE *values_per_segment = malloc(sizeof(VALUE_TYPE) * size), *current_value;
    VALUE_TYPE const *current_summarization;
    ...
    while ((current_segment = __sync_fetch_and_add(breakpointsCache->shared_processed_counter, 1)) < num_segments) {
        ...
        qsort(values_per_segment, size, sizeof(VALUE_TYPE), VALUE_COMPARE);
...
void profileBreakpoints(VALUE_TYPE const *breakpoints, unsigned int num_segments) {
    for (unsigned int i = 0; i < num_segments; ++i) {
        VALUE_TYPE sum = 0, sum_square = 0;
...
VALUE_TYPE const *getAdhocBreakpoints8(VALUE_TYPE const *summarizations, size_t size, unsigned int num_segments,
    ...
    VALUE_TYPE *breakpoints = aligned_alloc(64, sizeof(VALUE_TYPE) * OFFSETS_BY_SEGMENTS[num_segments]);
...
VALUE_TYPE const *getSharedAdhocBreakpoints8(VALUE_TYPE const *summarizations, size_t size, unsigned int num_segments) {
    ...
    VALUE_TYPE *values = malloc(sizeof(VALUE_TYPE) * size * num_segments);
    memcpy(values, summarizations, sizeof(VALUE_TYPE) * size * num_segments);
    qsort(values, size, sizeof(VALUE_TYPE), VALUE_COMPARE);
...
VALUE_TYPE const *getNormalBreakpoints8(unsigned int num_segments) {
    VALUE_TYPE *breakpoints = aligned_alloc(256, sizeof(VALUE_TYPE) * OFFSETS_BY_SEGMENTS[num_segments]);

    extractBreakpoints8(breakpoints, NORMAL_BREAKPOINTS_8, 255);
    for (unsigned int i = 1; i < num_segments; ++i) {
        memcpy(breakpoints + OFFSETS_BY_SEGMENTS[i], breakpoints, sizeof(VALUE_TYPE) * OFFSETS_BY_CARDINALITY[8]);
    }
...
int dumpBreakpoints8(const char *breakpoints_dump_filepath, VALUE_TYPE const *breakpoints, unsigned int num_segments) {
    FILE *bp_file = fopen(breakpoints_dump_filepath, "wb");
    fwrite(breakpoints, sizeof(VALUE_TYPE), OFFSETS_BY_SEGMENTS[num_segments], bp_file);
    return fclose(bp_file);
}

int loadBreakpoints8(const char *breakpoints_load_filepath, VALUE_TYPE const *breakpoints, unsigned int num_segments) {
    VALUE_TYPE *breakpoints2load = (VALUE_TYPE *) breakpoints;
    FILE *bp_file = fopen(breakpoints_load_filepath, "rb");
    size_t nitems = fread(breakpoints2load, sizeof(VALUE_TYPE), OFFSETS_BY_SEGMENTS[num_segments], bp_file);
    assert(nitems == OFFSETS_BY_SEGMENTS[num_segments]);
    return fclose(bp_file);
}
